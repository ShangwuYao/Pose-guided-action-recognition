...
class JHMDB_image(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(np.array(self.data['image'][index])).float(), \
            ...
            torch.FloatTensor(self.data['mask'][index].astype('float')), \
            pose_data, \
...
class LSTMModel(torch.nn.Module):
    ...
    def forward(self, input_features):
        ...
        #pose_logits_with_extracted_features = self.dropout_layer(torch.cat((extracted_features, pose_logits), 1))
    ...
    def model_train(self, freeze=True, use_mask=False, use_pose=False):
        ...
        for i in range(self.args.start_epoch, self.args.epochs):
            ...
            for features, action_label, mask, pose, scale in self.dataloader: # TODO: change back
                ...
                if use_pose:
                    ...
                    for scale_idx, pose_15 in enumerate(pose):
                        ...
                        for idx, pose_pt in enumerate(pose_15):
                            ...
                            pose_heatmap[idx] = circle(img, pose_pt, 1, self.args.radius * scale[scale_idx])
                    ...
                    #print("pose_heatmaps", pose_heatmaps)
                ...
                action_score = self.forward(features)
            ...
            self.model_param_str = \
                'att_img_nopose_epoch_{}_loss_{}_valloss_{}'.format(
    ...
    def evaluate(self, use_mask=False):
        ...
        true = 0
        false = 0
        for features, action_label, mask, _, _ in self.valid_dataloader:
            # change to frame level prediction
...
print("--training")
model.model_train(freeze=True, use_mask=False, use_pose=False)
...
model.model_train(freeze=False, use_mask=False, use_pose=False)
